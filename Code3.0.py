...
fs = [f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f1,f2,f2,f2,f3,f3]
...
class model:
	def __init__(self,n):
		...
		self.f = fs[random.randint(0,19)]
		self.b = [random.random() for i in range(self.n_len-1)]
	...
	def gent2(self):
		for i in range(len(self.get_list)-1):
			...
			for a in range(len(num)):
				...
				self.new_mdn[self.get_list[a]].b = self.mdn[self.get_list[a]].b[0:self.yb]+self.mdn[self.get_list[rn]].b[self.yb:self.n_len]
				self.new_mdn[self.get_list[rn]].b = self.mdn[self.get_list[rn]].b[0:self.yb]+self.mdn[self.get_list[a]].b[self.yb:self.n_len]
				self.new_mdn[self.get_list[a]].f = self.mdn[self.get_list[a]].f
				self.new_mdn[self.get_list[rn]].f = self.mdn[self.get_list[a]].f
...
mds = models(1000,[1,5,5,1],lc=3) 
for i in range(200):
	got1 = random.random()
	got2 = got1*3+0.1 #function
	mds.go([[got1]],[[got2]])
	mds.print_test(100)
...
print(a*3+0.1) #Expected Output
#for i in range(100):
#	print(mds.mdn[mds.test_number[0][1]].forward([i]))
