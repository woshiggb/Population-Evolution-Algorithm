...
class model:
	def __init__(self,n):
		...
		self.b = random.random()*random.randint(-1,1)
		self.go = [[0 for j in range(n[i])] for i in range(len(n))]
	...
	def gent2(self):
		for i in range(len(self.get_list)-1):
			...
			for a in range(len(num)):
				...
				if (random.random() >= 0.9 and self.mdn[self.get_list[a]].test_num > 0.1):
					self.new_mdn[self.get_list[a]].b = random.randint(-1,1)*random.random()
				if (random.random() >= 0.90 and self.mdn[self.get_list[rn]].test_num > 0.1):
					self.new_mdn[self.get_list[rn]].b = random.randint(-1,1)*random.random()
				i+=1
...
og = [1,20,20,1] #Model size (input, hidden layer 1, hidden layer 2, ......, output)
#lc Number of next Generation of Undead
mds = models(num,og,lc=3)
for i in range(20):
	got1 = random.random()
	got2 = got1/5
	mds.go([[got1]],[[got2]])
	mds.print_test(100) #First 100
print(mds.mdn[0].w) #w
print(mds.mdn[0].forward([0.5])) #print forward
print(mds.mdn[0].test([[0.5]],[[0.1]])) #error
